...
!приписка индексов обязательная, чтобы в случае изменения файла программа знала чему какой индекс соотвтствует
---
...
!В Go это обычно **2 файла**:

- `greeter.pb.go` — **сообщения + метаданные proto** (protobuf runtime)
    
- `greeter_grpc.pb.go` — **RPC слой** (клиент/сервер, регистрация, хендлеры)
---
